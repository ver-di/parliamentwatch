...
do
	...
	if [ $MULTISITE = "Y" -o $SITE = $SITE_INSTALL ]
		then
			...
			drush en --yes --uri=$SITE admin_devel admin_menu admin_menu_toolbar block_class ctools viewfield views_php page_manager views_content colorbox content_menu context context_layouts context_ui custom_search custom_search_blocks date date_all_day date_api date_migrate date_popup date_tools delete_all diff entityreference features_tools feeds feeds_ui feeds_comment_processor feeds_xpathparser devel entity_translation filefield_sources migrate migrate_extras migrate_ui rate votingapi ds ds_extras ds_search features fe_block uuid node_export node_export_features uuid_services uuid_features addressfield field_group link simplehtmldom file_entity translation media i18n_field i18n i18n_string i18n_taxonomy i18n_translation i18n_menu i18n_block i18n_field_group i18n_variable backup_migrate better_formats chart entity entity_token feedback_simple forward inline_messages job_scheduler libraries linkit masquerade menu_position module_filter nice_menus menu_import pathauto path_breadcrumbs path_breadcrumbs_ui read_more strongarm subform submenutree text_resize textformatter token user_revision users_export panels php print role_export rules rules_scheduler rules_admin search_api search_api_solr search404 sharethis facetapi secureshare secureshare_fields taxonomy_csv de_stemmer stemmer_api tagadelic delta delta_blocks delta_ui omega_tools compact_forms wysiwyg variable variable_realm variable_store views views_data_export views_php views_slideshow views_slideshow_cycle views_ui webform webform_term_opts webform_rules weight search_api_views search_api_db page_title
			...
			drush dis --yes --uri=$SITE overlay toolbar
