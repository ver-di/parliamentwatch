<?php
  ...
  // Starting script ///////////////////////////////////////////////////////////
  watchdog("PW_TEST", t("Checking parliament term data integrity."), $variables = NULL, WATCHDOG_DEBUG, $link = NULL);
  ...
  if (!$dblink = mysql_connect('localhost', 'root', 'quickstart')) {
    watchdog("PW_TEST", t("Could not connect to mySQL database!"), $variables = NULL, WATCHDOG_ERROR, $link = NULL);
    exit;
  }
  // Migration database name ///////////////////////////////////////////////////
  if (!mysql_select_db('parlamentwatch', $dblink)) {
    watchdog("PW_TEST", t('Could not select "parlamentwatch" database!'), $variables = NULL, WATCHDOG_ERROR, $link = NULL);
    exit;
  }
  ...
  $sql = 'SELECT ' . $dbSelect . ' FROM ' . $dbFrom . ' WHERE project_cmd '
       ...
       . ' BETWEEN 0 AND 2000 GROUP BY ' . $dbSelect . ' ORDER BY ' . $dbSelect . ' ASC';
  $result = mysql_query($sql, $dblink);
  if (!$result) {
    watchdog("PW_TEST", t('Could not query the database!'), $variables = NULL, WATCHDOG_ERROR, $link = NULL);
    watchdog("PW_TEST", t('MySQL Error: ' . mysql_error()), $variables = NULL, WATCHDOG_ERROR, $link = NULL);
    exit;
  }
  
  // Counting results in mirgration database ///////////////////////////////////
  watchdog("PW_TEST", t('Looking in DB for "' . $dbSelect . '" in "' . $dbFrom . '".'), $variables = NULL, WATCHDOG_INFO, $link = NULL);
  $i = 0;
  while ($row = mysql_fetch_assoc($result)) $i++;
  watchdog("PW_TEST", t('Found ' . $i . ' parliament terms in DB.'), $variables = NULL, WATCHDOG_NOTICE, $link = NULL);
  
  // Counting results in XML-API ///////////////////////////////////////////////
  watchdog("PW_TEST", t('Looking in XML for "' . $xmlExpression . '" in "' . $xmlPath . '".'), $variables = NULL, WATCHDOG_INFO, $link = NULL);
  $file = file_get_contents($xmlPath);
  $xml = simplexml_load_string($file);
  $xpath = $xml->xpath($xmlExpression);
  $j = 0;
  while(list(, $node) = each($xpath)) $j++;
  watchdog("PW_TEST", t('Found ' . $j . ' parliament terms in XML-API.'), $variables = NULL, WATCHDOG_NOTICE, $link = NULL);

  // Counting results in Taxonomy ///////////////////////////////////////////////
  watchdog("PW_TEST", t('Looking in taxonomy for "parliaments".'), $variables = NULL, WATCHDOG_INFO, $link = NULL);
  $vid = taxonomy_vocabulary_machine_name_load("parliaments")->vid;
  $terms = taxonomy_get_tree($vid);
  $numTerms = count($terms);
  watchdog("PW_TEST", t('Found ' . $numTerms . ' parliament terms in taxonomy.'), $variables = NULL, WATCHDOG_NOTICE, $link = NULL);
  
  // Checking for missing entries in Taxonomy //////////////////////////////////
  watchdog("PW_TEST", t('Checking missing entries in taxonomy...'), $variables = NULL, WATCHDOG_INFO, $link = NULL);
  $result = mysql_query($sql, $dblink);
  $countMissingTax = 0;
  while ($row = mysql_fetch_assoc($result)) {
    ...
    if (!$isMatch) {
      watchdog("PW_TEST", t('No match for: ' . utf8_encode($row[$dbSelect])), $variables = NULL, WATCHDOG_WARNING, $link = NULL);
      $countMissingTax++;
    }
  }
  $tmpString = '0';
  ...
  watchdog("PW_TEST", t($tmpString . ' missing entries in taxonomy.'), $variables = NULL, $status, $link = NULL);

  // Checking for missing entries in XML ///////////////////////////////////////
  watchdog("PW_TEST", t('Checking missing entries in XML...'), $variables = NULL, WATCHDOG_INFO, $link = NULL);
  $result = mysql_query($sql, $dblink);
  $countMissingXML = 0;
  while ($row = mysql_fetch_assoc($result)) {
    $isMatch = false;
  ...
  watchdog("PW_TEST", t($tmpString . ' missing entries in XML.'), $variables = NULL, $status, $link = NULL);
  
  // Checking for duplicates in XML ////////////////////////////////////////////
  watchdog("PW_TEST", t('Checking for duplicates in XML...'), $variables = NULL, WATCHDOG_INFO, $link = NULL);
  ...
  while (list(, $outterNode) = each($outterXpath)) {
    ...
    while (list(, $innerNode) = each($innerXpath)) {
      ...
      if (strcmp($outterNode, $innerNode) == 0 && $s != $t) {
        watchdog("PW_TEST", t('Possible duplicate entry for: ' . $innerNode . ' and ' . $outterNode), $variables = NULL, WATCHDOG_WARNING, $link = NULL);
        $countDuplicateXML++;
      }
    }
  }
  ...
  while(list(, $node) = each($xpath)) {
    ...
    if (!$isMatch) {
      watchdog("PW_TEST", t('No match for: ' . $node), $variables = NULL, WATCHDOG_WARNING, $link = NULL);
      $countMissingDB++;
    }
  }
  $tmpString = '0';
